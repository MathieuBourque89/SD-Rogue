...
public class Level : Scene {
   ...
   public Level(Player p, string map, Game game) {
    ...
    private void buildTraps()
    {
       ...
       {
          var pos = _floor.ElementAt(rng.Next(_floor.Count));
          _specialTiles.Add(new Trap(pos, rng.Next(3, 8)));
       }
    }
    ...
    protected void updateDiscovered()
    {
        ...
        _onPlayer = fovCalc(_player!.Pos, _senseRadius - 3);
...
// -------------------------------------------------------------------------
   ...
   // Mathieu's version of hiding items
   /*foreach (var item in _items) 
    ...
    private void drawSpecialTiles(IRenderWindow disp)
    {
       ...
       foreach (var tile in _specialTiles)
       {
          if (_inFov.Contains(tile.Pos))
             tile.Draw(disp);
          if (_onPlayer.Contains(tile.Pos))
          {
             _player._hp = _player._hp - rng.Next(3, 8);
